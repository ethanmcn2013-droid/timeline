...
## 2026-06-30 · R·27 · tightens · Timeline's footer now matches the public suite frame

**The public footer now follows the same Product, Company, Resources, Suite structure as the rest of Signal Studio.** Timeline keeps its own product links, the company and legal paths point back to Studio-owned surfaces, and the Suite column names Signal Studio, Signal Notes, Signal Tasks, Signal Timeline, and Signal in one consistent order. The new `iOS app` link goes to Studio's honest `/ios` status page while the native app is still in build. Typecheck and production build are clean.
