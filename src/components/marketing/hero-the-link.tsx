/**
 * Timeline hero — "The Link".
 *
 * Graduated from the hero lab (feat/timeline-hero-lab, direction 2 of 3)
 * on 2026-07-07. The URL is the product, so the URL is the hero. The shared
 * link sits in a real browser (tab, nav, favicon) and opens with no wall in
 * front of it: the plan unfolds exactly as the person on the other end sees
 * it. No account, no app. The path types itself into the bar, the sign-in
 * wall dissolves to reveal the already-loaded plan, and a "Copy link →
 * Copied" beat shows how it travels.
 *
...
.tl2-browser{position:relative;min-width:0;border:1px solid var(--hair);border-radius:14px;overflow:hidden;background:var(--paper);
...
.tl2-omni{flex:1;min-width:0;overflow:hidden;display:flex;align-items:center;gap:8px;height:34px;padding:0 12px;background:#f6f6f7;
...
/* Small screens: the browser keeps its shape. Nav arrows and the copy
   button step aside so the URL, the product truth, stays readable. */
@media (max-width:560px){
  .tl2-nav{display:none}
  .tl2-share{display:none}
  .tl2-omni{font-size:11.5px}
  .tl2-page{padding:22px 18px 22px}
  .tl2-row{grid-template-columns:64px 14px 1fr auto;gap:10px}
  .tl2-lanes::before{left:80px}
}
