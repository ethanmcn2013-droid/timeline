...
/**
 * Timeline hero — "The Line".
 *
 * Graduated from the hero lab (feat/timeline-hero-lab, direction "The Line")
 * on 2026-07-17, replacing "The Link" as the homepage opener. The editorial
 * rule is present from frame zero: three short sentences name the job, then a
 * single horizontal line reads left to right and the real dated plan settles
 * onto it — Now, Soon, Later, Done. Restraint with a product argument: the plan
 * is one public line anyone can read.
 *
 * Pure CSS, server-rendered. Default styles ARE the rest state (the settled
 * plan); the intro sequence plays once and only under
 * `prefers-reduced-motion: no-preference`. Scoped `tl1-`.
 */
