  ...
  return (
    <main className="labx">
      <header className="labx-head">
        ...
        <h1 className="labx-h1">Five directions. Two front-runners.</h1>
        <p className="labx-lede">
          One Line is the flagship. The Line is its quieter editorial counterpoint.
          Three sharing-story candidates remain available for comparison. Watch the
          sentence, the product mechanism, and the settled artifact. Press 1 to {OPTIONS.length} to
          jump, or R to replay.
        </p>
      </header>
...
const INDEX_CSS = `
.labx{min-height:100svh;background:var(--paper);color:var(--ink);font-family:var(--font-geist-sans,system-ui,sans-serif);
...
.labx-lede{font-size:17px;line-height:1.55;color:var(--ink-soft);max-width:68ch;margin:0 0 44px}
...
.labx-card{display:flex;flex-direction:column;gap:6px;padding:22px;border:1px solid rgba(17,17,17,.1);
  border-radius:14px;text-decoration:none;color:inherit;transition:.15s;background:var(--paper)}
.labx-card:hover{border-color:var(--accent);transform:translateY(-2px);box-shadow:0 10px 30px -18px var(--accent-glow)}
...
.labx-idx{display:grid;place-items:center;width:24px;height:24px;border-radius:50%;
  border:1px solid rgba(17,17,17,.18);font-family:var(--font-geist-mono,monospace);font-size:12px;color:var(--ink-faint)}
.labx-role{font-family:var(--font-geist-mono,monospace);font-size:10.5px;letter-spacing:.08em;
  text-transform:uppercase;padding:3px 8px;border-radius:999px}
.labx-role-preferred{color:var(--paper);background:var(--accent)}
.labx-role-candidate{color:var(--ink-soft);background:var(--paper-deep)}
...
.labx-lens{font-family:var(--font-geist-mono,monospace);font-size:11px;letter-spacing:.04em;
  color:var(--ink-ghost);text-transform:uppercase;margin:0}
.labx-headline{font-size:14px;color:var(--ink);margin:4px 0 0;font-style:italic}
.labx-blurb{font-size:13.5px;line-height:1.5;color:var(--ink-faint);margin:2px 0 0;flex:1}
.labx-open{font-size:13px;color:var(--accent);margin-top:8px}
.labx-card:focus-visible{outline:2px solid var(--accent);outline-offset:3px}
@media (prefers-reduced-motion:reduce){.labx-card{transition:none}.labx-card:hover{transform:none}}
`;
